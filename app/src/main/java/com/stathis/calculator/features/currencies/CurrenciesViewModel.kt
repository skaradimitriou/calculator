...
class CurrenciesViewModel : ViewModel() {
    ...
    fun getRates(amount: Double, currencyFrom: String, currencyTo: String) {
        val json =
    ...
    private fun calculateRate(
        ...
        getFinalResult(currencyRateFrom, currencyRateTo, amount)
    }

    private fun getFinalResult(currencyRateFrom: Double, currencyRateTo: Double, amount: Double) {
        val x = currencyRateTo * (1 / currencyRateFrom)
        val result = x * amount
        data.value = result.toString()

        //FIXME: Add currencyCodes as well
        equation.value = "$amount $currencyRateFrom = $result $currencyRateTo"
    }
